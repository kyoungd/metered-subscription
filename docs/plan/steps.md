...
1. **Architect AI** — *fills only `<…>` fields* in the pre-plan template.
   * No repo access, no `(Cursor Plan)` fields, no compact JSON yet.
2. **Human Review (Gate A)** — checks the filled `<…>` only.
   * Scope, METHOD PATH, acceptance (Baseline 5 + relevant situational), security/headers/idempotency/SoT/time, rollback, LOC budget, code-boundary fit.
3. **Cursor AI · Plan Mode** — *fills only `(Cursor Plan)` fields*.
   * Proposes `touch_set` (+ rationale) and `reuse_first` (+ rationale) as `"proposed"`.
4. **Human Review (Gate B)** — trims to minimal, flips both modes to `"locked"`; enforces Appendix A.
5. **Compact JSON artifact** — generated **after** Cursor’s review (post-Gate B), for handoff to implementation.
---
