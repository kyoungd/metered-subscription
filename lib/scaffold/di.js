...
export class Container {
  ...
  constructor(env) {
    ...
    // Create app-scope HTTP client
    this.httpClient = http(env)

    // Register app-scope client factories
    this.clientFactories = {
      stripe: ({ env, call_state }) =>
        createStripeClient({ env, call_state, http: this.httpClient }),
    }
  }
  ...
  createRequestContext(headers) {
    ...
    const clients = {
      stripe: this.clientFactories.stripe({ env: this.env, call_state }),
    }
